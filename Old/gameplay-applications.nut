...
SetScriptVersion("GameplayApplications", "2.8.1")
...
::GameplayEvents <- {
	...
	function OnScriptHook_OnTakeDamage(params)
	{
		...
		if(!hAttacker || !hWeapon || !hInflictor)
			return;

		if ( !(startswith(hWeapon.GetClassname(), "tf_weapon") || startswith(hWeapon.GetClassname(), "tf_wearable"))) return
		if ( hVictim.GetClassname() == "tf_zombie") { params.damage = MATH.Min(params.damage, 5.0) ; return }
		...
		switch (params.damage_stats)
		{
			...
			case TF_DMG_CUSTOM_KART: //BUMPER CAR
			{
				...
				hVictim.TakeDamageCustom(null, params.attacker, null, params.damage_force, params.damage_position, KART_DMG, params.damage_type, TF_DMG_CUSTOM_TRIGGER_HURT)
				return
			}
			case TF_DMG_CUSTOM_SPELL_SKELETON: //SKELETONS
			{
				params.damage = 8500
				if (hVictim.IsPlayer()) hVictim.AddCondEx(TF_COND_MARKEDFORDEATH, 10, hAttacker)
				break
			}
			...
			case TF_DMG_CUSTOM_SPELL_TELEPORT: //TELEPORT
			{
				params.damage = 10000 // ?
				break
			}
		...
		switch (hWeapon.GetIDX())
		{
			case TF_WEAPON_TOMISLAV:
			{
				...
				scope.m_flLastHeatHit <- Time()
				foreach (attribs in TOMISLAV_SETTINGS.Attributes)
					hWeapon.CalculateAttributeChange(scope.Hits, attribs[0], attribs[1], attribs[2], attribs[3], attribs[4])
			...
			{
				if( !hVictim.IsPlayer() || !hVictim.IsValidReprogramTarget() || hVictim.GetPlayerClass() == TF_CLASS_MEDIC || hVictim.GetTeam() == TF_TEAM_RED)
				{
					hAttacker.GetWeaponInSlotNew(SLOT_SECONDARY).IncreaseUberChargePercent(BlutsaugerSettings.refund)
					if(!hVictim.IsPlayer())
						return
					if(!hVictim.IsValidReprogramTarget())
						hAttacker.TranslateToChat("REPROG_BOT_STRONG", hVictim.GetUserName())
					else if(hVictim.GetPlayerClass() == TF_CLASS_MEDIC)
						hAttacker.TranslateToChat("REPROG_BOT_NOT_SUIT", hVictim.GetUserName())
					
					return;
				}

				TranslateToChatAll("REPROG_BOT_MESSAGE", hAttacker.GetUserName(), hVictim.GetUserName())

				GetScope(hVictim).ReProgrammer <- hAttacker
			...
			{
				if( !hVictim.IsPlayer() || !(params.damage_type & DMG_ACID) || !hVictim.CanHaveCorrosion() || hVictim.HasCorrosion() )
					return
				
				// hAttacker.PrintToHud("Made Corrosion on " + hVictim)
				EmitGlobalSound({
					sound_name = ""
					entity = hVictim
					sound_level = 80
